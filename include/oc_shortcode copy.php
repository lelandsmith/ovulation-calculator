...
<?php function check_available_date($firstday, $next_period, $selected_period_date){
	...
	for($month_count = 1; $month_count<=6; $month_count++){ // 6 months result
		...
		$add_days = $_POST['days']-4; // minus 5 from selected cycle 
...
if(!empty($_POST['calculator_ok'])):
	...
	<div class="calculator_table">
		...
		<div class="email-area">
			...
			<form action="https://online.us16.list-manage.com/subscribe/post?u=57dbd21e405e6ca50bba6ab64&amp;id=df02381e22" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form"  autocomplete="on">
				<div class="email-box">
				<input type="email" name="EMAIL" id="mce-EMAIL" placeholder="<?php printf(__('%s', 'ovulation-calculator'), $options['oc-enter-email']);?>"/>
				<?php if(!empty($options['oc-download-message'])):
					printf(__('<p>%s</p>', 'ovulation-calculator'), $options['oc-download-message']);
				endif;?>
				
				<div style="position: absolute; left: -5000px;" aria-hidden="true"><input type="text" name="b_57dbd21e405e6ca50bba6ab64_df02381e22" tabindex="-1" value=""></div>
				...
				<div class="submit-btn">
					<button type="submit" name="subscribe" id="mc-embedded-subscribe" class="button"><?php printf(__('%s', 'ovulation-calculator'), $options['oc-email-send']);?><i class="fa fa-angle-right fa-2x" aria-hidden="true"></i></button>
					
<!-- 					<button type="submit" name="subscribe" id="mc-embedded-subscribe" value="<?php printf(__('%s', 'ovulation-calculator'), $options['oc-email-send']);?>"/> --></div>
			</div>
			</form>
		</div>
	</div>
